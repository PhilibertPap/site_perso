...
const VIEWPORT_DESKTOP = "width=1100, viewport-fit=cover";
...
const CONTENT_TRANSLATIONS = {
  en: {
    ...
    description: {
      ...
      "Étude analytique puis numérique du problème": "Analytical then numerical study of the problem",
      ...
      "Aujourd’hui, je joue occasionnellement lors de messes en semaine ou de mariages dans la famille — sans prétention de virtuose, mais avec assez d’aisance pour tenir la partie d’accompagnement.": "Today I play occasionally at weekday masses or family weddings — without any claim to virtuosity, but with enough ease to hold the accompaniment part.",
      ...
      "Je m'intéresse aussi beaucoup à l'histoire des États allemands et de l'Autriche.": "I am also very interested in the history of the German states and Austria.",
  ...
  de: {
    ...
    description: {
      ...
      "Étude analytique puis numérique du problème": "Analytische und anschließend numerische Untersuchung des Problems",
      ...
      "Aujourd'hui, je joue occasionnellement lors de messes en semaine ou de mariages dans la famille — sans prétention de virtuose, mais avec assez d'aisance pour tenir la partie d'accompagnement.": "Heute spiele ich gelegentlich bei Wochenmessen oder Familienhochzeiten — ohne Anspruch auf Virtuosität, aber mit genug Sicherheit, um die Begleitstimme zu übernehmen.",
      ...
      "Je m'intéresse aussi beaucoup à l'histoire des États allemands et de l'Autriche.": "Ich interessiere mich außerdem sehr für die Geschichte der deutschen Staaten und Österreichs.",
...
const CONTENT_TRANSLATIONS_PATCH = {
  "en": {
    "description": {
      ...
      "Formulation et résolution de problèmes d'élasticité en petites et grandes déformations, avec introduction aux principes variationnels et aux méthodes énergétiques.": "Formulation and resolution of elasticity problems in small and large deformations, with introduction to variational principles and energy methods.",
      ...
      "Une partie est consacrée à l’implémentation numérique des lois de comportement et au passage du matériau à la structure sur des cas de chargements thermomécaniques.": "A part is devoted to the numerical implementation of constitutive laws and the transition from material to structure for thermomechanical loading cases.",
      ...
      "Il développe ensuite la méthode des éléments finis, de la formulation variationnelle à la discrétisation, puis ses étapes de mise en œuvre : maillage, fonctions de forme, intégration numérique, assemblage et post-traitement.": "It then develops the finite element method, from variational formulation to discretization, then its implementation stages: meshing, shape functions, numerical integration, assembly and post-processing.",
      ...
      "Des thèmes avancés sont abordés, notamment la dynamique des milieux structurés et la dynamique non linéaire, avec un appui fort sur les PC et l’implémentation numérique en Python.": "Advanced themes are covered, notably the dynamics of structured media and nonlinear dynamics, with strong support from practical sessions and numerical implementation in Python.",
      ...
      "Produit tensoriel, intrication et spin 1/2, de l’expérience de Stern–Gerlach à la résonance magnétique nucléaire.": "Tensor product, entanglement and spin 1/2, from the Stern–Gerlach experiment to nuclear magnetic resonance.",
      ...
      "Le cours articule lecture comparée d'œuvres philosophiques et littéraires, analyse conceptuelle et entraînement méthodique à la dissertation.": "The course combines comparative reading of philosophical and literary works, conceptual analysis and methodical dissertation training.",
      ...
      "Le cours combine étude d'œuvres, clarification des concepts, confrontation des positions d'auteurs et entraînement intensif à l'écrit.": "The course combines study of works, clarification of concepts, comparison of authors' positions and intensive writing training.",
  ...
  "de": {
    "description": {
      ...
      "Formulation et résolution de problèmes d'élasticité en petites et grandes déformations, avec introduction aux principes variationnels et aux méthodes énergétiques.": "Formulierung und Lösung von Elastizitätsproblemen bei kleinen und großen Verformungen, mit Einführung in Variationsprinzipien und Energiemethoden.",
      ...
      "Il développe ensuite la méthode des éléments finis, de la formulation variationnelle à la discrétisation, puis ses étapes de mise en œuvre : maillage, fonctions de forme, intégration numérique, assemblage et post-traitement.": "Der Kurs entwickelt dann die Finite-Elemente-Methode, von der Variationsformulierung bis zur Diskretisierung, dann ihre Implementierungsstufen: Vernetzung, Formfunktionen, numerische Integration, Assemblierung und Nachbearbeitung.",
      ...
      "Des thèmes avancés sont abordés, notamment la dynamique des milieux structurés et la dynamique non linéaire, avec un appui fort sur les PC et l’implémentation numérique en Python.": "Es werden fortgeschrittene Themen behandelt, insbesondere die Dynamik strukturierter Medien und die nichtlineare Dynamik, mit starker Unterstützung durch Übungen und numerischer Implementierung in Python.",
      ...
      "Le cours articule lecture comparée d'œuvres philosophiques et littéraires, analyse conceptuelle et entraînement méthodique à la dissertation.": "Der Kurs kombiniert vergleichende Lektüre philosophischer und literarischer Werke, konzeptionelle Analyse und methodisches Dissertationstraining.",
      ...
      "Le cours combine étude d'œuvres, clarification des concepts, confrontation des positions d'auteurs et entraînement intensif à l'écrit.": "Der Kurs verbindet Werkstudium, Konzeptklärung, Vergleich der Autorenpositionen und intensives Schreibtraining.",
...
function getStoredViewMode() {
  try {
    return localStorage.getItem(VIEW_MODE_STORAGE_KEY) === "desktop" ? "desktop" : "mobile";
  } catch (e) {
    return "mobile";
  }
}
...
function setStoredViewMode(mode) {
  try {
    localStorage.setItem(VIEW_MODE_STORAGE_KEY, mode === "desktop" ? "desktop" : "mobile");
  } catch (e) {}
}

function updateViewModeButton() {
  const btn = document.querySelector("[data-view-mode-toggle]");
  if (!btn) return;
  const lang = document.documentElement.getAttribute("lang") || "fr";
  const ui = getUi(lang);
  const footer = (ui && ui.footer) || {};
  const forced = document.documentElement.classList.contains("force-desktop-view");
  const label = forced
    ? footer.switch_to_mobile || "Version mobile"
    : footer.switch_to_desktop || "Version ordinateur";
  const labelEl = btn.querySelector("[data-view-mode-label]");
  const iconEl = btn.querySelector(".view-mode-icon");
  if (labelEl) labelEl.textContent = label;
  if (iconEl) iconEl.textContent = forced ? "\uD83D\uDCF1" : "\uD83D\uDDA5\uFE0F";
  btn.setAttribute("aria-label", label);
}

function applyViewMode(mode) {
  const forced = mode === "desktop";
  const meta = document.getElementById("viewport-meta");
  if (meta) meta.setAttribute("content", forced ? VIEWPORT_DESKTOP : VIEWPORT_MOBILE);
  document.documentElement.classList.toggle("force-desktop-view", forced);
  updateViewModeButton();
}
...
function initViewMode() {
  ...
  applyViewMode(isMobile ? getStoredViewMode() : "mobile");
  if (btn) {
    if (isMobile) btn.setAttribute("aria-hidden", "false");
    btn.addEventListener("click", function () {
      const next = document.documentElement.classList.contains("force-desktop-view")
        ? "mobile"
        : "desktop";
